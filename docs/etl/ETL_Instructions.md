...
| Argument | Type | Description |
|----------|------|-------------|
| *variableName* | `String` | The variable name is the variable that will receive the value input by the user. It is also used to fetch the default value in the event the user just hits return. |
| *nativeType* | `MTNativeType` | The data type for the variable that is to receive the user's response. If the value doesn't match the type, it will through error and ask again. |
